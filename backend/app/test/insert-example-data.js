...
doctors.forEach(doctor => {
  const req = http.request(options, (res) => {
    ...
    res.on('end', () => {
      console.log(`Inserted: ${doctor.Gpname} - Status: ${res.statusCode}`);
    });
  });

  req.on('error', (error) => {
    console.error(`Error inserting ${doctor.Gpname}: ${error.message}`);
  });

  // Send the doctor data as a JSON string
  req.write(JSON.stringify(doctor));
  req.end();
});
